...
public sealed partial class ExprParser(ParserContext ctx, PatternParser patternParser, TypeParser typeParser)
{
    ...
    private bool TryParseAssociatedConstExpr(EidosAstNode left, out AssociatedConstExpr associatedConst)
    {
        ...
        associatedConst.SetSpan(CreatePostfixSpan(left, startToken));
    ...
    private GivenExpr ParseGivenExpr(EidosAstNode target)
    {
        ...
        given.SetSpan(CreatePostfixSpan(target, startToken));
    ...
    private IndexExpr ParseIndexAccess(EidosAstNode target)
    {
        ...
        indexExpr.SetSpan(CreatePostfixSpan(target, startToken));
    ...
    private RecordUpdateExpr ParseRecordUpdate(EidosAstNode baseExpression)
    {
        ...
        ctx.Expect("}");
        update.SetSpan(CreatePostfixSpan(baseExpression, startToken));
        return update;
    }
    ...
    private EidosAstNode ParseFieldAccess(EidosAstNode left)
    {
        ...
        methodCall.SetSpan(CreatePostfixSpan(left, startToken));
    ...
    private EidosAstNode ParseTypeArgAccess(EidosAstNode left)
    {
        ...
        if (left is PathExpr pathExpr)
        {
            ...
            pathExpr.SetSpan(CreatePostfixSpan(left, startToken));
            return pathExpr;
        }

        var typeApplication = new IndexExpr();
        typeApplication.SetSpan(CreatePostfixSpan(left, startToken));
    ...
    private EidosAstNode ParseInfixBacktick(EidosAstNode left)
    {
        ...
        var infixCall = new InfixCallExpr();
        infixCall.SetSpan(CreatePostfixSpan(left, startToken));
        infixCall.SetLeft(left);
    ...
    private SourceSpan CreatePostfixSpan(EidosAstNode left, Token fallbackStart)
    {
        var start = left.Span.Position >= 0 ? left.Span.Location : fallbackStart.Location;
        var end = ctx.Peek(-1);
        return new SourceSpan(start, end.Location.Position + end.Length - start.Position);
    }
