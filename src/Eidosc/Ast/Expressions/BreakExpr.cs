...
/// break x
/// </example>
public record BreakExpr : Expression
{
    /// <summary>
    /// break 值表达式（可选）
    /// </summary>
    ...
    private static bool TryCreateExpressionFromTerminal(TerminalCstNode terminal, out EidosAstNode expression)
    {
        ...
        if (terminalName == WellKnownStrings.AdditionalKeywords.Break)
        {
            return false;
        }
    ...
    {
        var element = CreateElement(doc, WellKnownStrings.XmlElements.BreakExpr);
