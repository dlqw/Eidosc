...
- **驱动工具**：`tools/c2e`（dotnet；`--report` 出逐函数可行性矩阵，`--only` 选入口，
  `-I/-D` 编译环境）。依赖 `Eidosc/src/Eidosc/Properties/AssemblyInfo.cs` 里的
  `InternalsVisibleTo("C2eDriver")`。
- **翻译率现状**（真实 raylib 源，`PLATFORM_DESKTOP_WIN32 + GRAPHICS_API_OPENGL_33`）：
  rcore 638/1326、rshapes 62/127、rtext 319/628、rtextures 464/1156；raymath 159/174（91%）。
- **git 状态**：以下改动全部**未提交**（main 分支工作区）：
  - `src/Eidosc/Bindgen/Clang/CBodyTranslator.cs`（全部翻译器扩展，~1000 行增量）
  - `src/Eidosc/Bindgen/Clang/ClangNative.cs`（getAsDouble 导出）
  - `src/Eidosc/Properties/AssemblyInfo.cs`（InternalsVisibleTo）
  - `src/Eidosc.Tests/Integration/LlvmPipelineIntegrationTests.C2EParity.cs`
    （union 测试修正 + 新增 `C2E_StructValueBridge_ParityWithClang`；当前 6/6 绿）
  - `changelogs/0.9.0-alpha.2/2026-08-16-*.md` 两片
  - 工作区（repo 外）：`projects/bindings/raylib-c2e/`、`projects/snake-gui-c2e/`、`tools/c2e/`、
    `AGENTS.md` 注册行。
  - 历史遗留未提交：`2026-08-15-...-c2e-float-eval-and-shim-linkage.md`（getAsStr AV 修复）。
  - 建议新会话第一件事：审阅并按仓库惯例分支提交（squash 或分两个 PR：翻译器扩展 / demo 工程）。
...
## 2. L1 识别机制设计（核心思考题：C2E 如何知晓 OS 边界）
...
数据为四模块（rcore/rshapes/rtext/rtextures）全量 skip 聚合（2026-08-16，本轮修复后）：

| # | 阻塞 | 量 | 实施要点 |
|---|---|---|---|
| 1 | 数组下标 `a[i]`（指针算术） | 88+ | 见 §4.2（依赖语言侧 Ffi 指针算术）；翻译层：元素地址 = `Ffi.pointer_add(base, i * sizeof(T))` + `load/store[T]`；C 数组形参（`T a[]`/`T* a`）在 MapType 已落 RawPtr，缺的是下标表达式 |
| 2 | 调用链失败传播残留 | 176 | 多为 #1/#3 的下游，上游落地后自动消解；无需单独处理 |
| 3 | 位运算符 `& | ^ << >>` | 93 | 语言缺口（§4.1）；翻译层透传即可（操作数全 Int） |
| 4 | 字符串字面量 | 66 | 映射为 Eidos `String`；在 extern `RawPtr` 实参位自动 `Ffi.to_c_string(...)`（复用 `TranslateCallArgument` 的参数映射）；同文件内传给已翻函数的场景保持 String |
| 5 | `unsupported operator`（`<<`等） | 93 | 同 #3 |
| 6 | 一元 `&`（取地址）、`~` | 61 | `&x` → 局部地址：需语言侧取址（§4.3）或只支持 `&global`（模块 mut 的地址）；`~` 依赖 #3 |
| 7 | va_list（`TraceLog`） | 少量 | **策略性跳过**：翻译器对 va_list 原型已跳过；不建议支持，日志类函数留地板 |
| 8 | 嵌套成员赋值 `a.b.c = v` | 34 | 发射 `a := a.{b: a.b.{c: v}}`（需验证 Eidos 嵌套 record update 语法；先查 `TypeInferencePipelineTests` 与 snake-gui 是否已有用例） |
| 9 | 语句 kind 206（switch） | 少量 | 去糖为 `decide`/if-else 链：`case A: ... break;` → 分支；注意 fallthrough 与 `default`；无 break 的 fallthrough 用嵌套块拼接。键位映射表是主要用户 |
| 10 | extern 结构体按值 ABI（`rlGetMatrixTransform` 等） | ~20 | 生成 sret shim：C 侧 `void __c2e_sret_F(S* out){ *out = F(); }`，Eidos 侧 extern `Unit -> RawPtr` + 生成的字段读取拼装记录（复用 accessor 机制）。落地后 `DrawRectangleLines` 可真翻译，撤掉 drawing.eidos 的四矩形兜底 |

## 4. L3 Eidos 语言/编译器项

### 4.1 位运算符（**最高优先**，93 处阻塞的唯一语言级硬缺口）

- 现状：`SyntaxKind.cs` 无 `OpAmpersand/OpCaret/OpLessLess/OpGreaterGreater`（`OpPipe` 是管道）。
- 方案 A（推荐）：词法 + 语法 + 类型规则一步到位——`Int -> Int -> Int`，优先级同 C
  （`<< >>` 高于关系、`& ^ |` 依次降低）；`Bool` 不参与。AST/HIR/MIR/LLVM 全链路
  （`shl/lshr/and/or/xor` 指令），参考 #69（unsigned 算术）的落地路径。
- 方案 B（快但脏）：stdlib `Int.bit_and/bit_or/bit_xor/shl/shr` + 运行时 C 实现——
  可先行解 C2E 阻塞，语言层后补操作符糖。
- 需同步：`std.Hash` 等可能已有按位需求，检查 `stdlib` 内是否已有私有内建可复用。
...
1. `dotnet test src/Eidosc.Tests --filter "FullyQualifiedName~C2E_"`（当前 6/6）；
...
5. 全量回归一次（上轮基线 4184/4187，3 个已知失败对应 #83 模板迁移与 #85 flake）。

## 7. 新会话启动清单

```bash
cd /d/Project/eidos_workspace
# 1) 审阅未提交改动并提交（见 §0 git 状态）
git -C Eidosc status --short
# 2) 构建驱动
dotnet build tools/c2e
# 3) 复现基线
./Eidosc/tools/… # 无；直接：
...
# 4) 按优先级开工：§2 L1 识别 → §4.1 位运算 → §4.2 指针算术 → §3 清单
```
